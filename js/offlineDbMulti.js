...
window.offlineDbV3 = (function () {
    ...
    async function deleteDatabase(timeoutMs = 5000) {
        return new Promise((resolve, reject) => {
            ...
            // El timeout tiene que cortar SIEMPRE, no solo cuando llegó
            // 'blocked'. Hay navegadores que dejan la petición sin resolver y
            // sin disparar onblocked; si ahí no rechazamos, esta promesa no
            // termina nunca y el logout se queda colgado sin llegar al login.
            const timeout = setTimeout(() => {
                if (terminado) return;
                terminado = true;
                reject(new Error(bloqueado
                    ? 'No se pudo eliminar la base de datos porque hay otras pestañas abiertas. Por favor, ciérralas e inténtalo de nuevo.'
                    : 'No se pudo eliminar la base de datos: la operación no respondió a tiempo.'));
            }, timeoutMs);
